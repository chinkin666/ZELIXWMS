...
// 日本郵便の送り状種類（静的参照データ） / 日本邮便送状类型（静态参考数据）
const INVOICE_TYPES = [
  { code: '1', name: 'ゆうパック / 邮包', description: '通常ゆうパック / 普通邮包' },
  { code: '2', name: 'ゆうパケット / 邮件包', description: 'ゆうパケット / 邮件小包' },
  { code: '3', name: 'ゆうメール / 邮件', description: 'ゆうメール / 邮件' },
  { code: '4', name: 'レターパック / 信件包', description: 'レターパックライト・プラス / 信件包' },
];

// 日本郵便CSVヘッダー定義 / 日本邮便CSV头定义
const JAPAN_POST_CSV_HEADERS = [
  'お届け先郵便番号',    // 收件人邮编
  'お届け先住所1',       // 收件人地址1
  'お届け先住所2',       // 收件人地址2
  'お届け先住所3',       // 收件人地址3
...
// 日本郵便追跡CSVカラムインデックス / 日本邮便追踪CSV列索引
const TRACKING_CSV_COLUMNS = {
  customerManagementNumber: 0,  // お客様管理番号 / 客户管理编号
  trackingNumber: 1,            // 追跡番号 / 追踪编号
};

@Injectable()
export class JapanPostService {
  constructor(@Inject(DRIZZLE) private readonly db: DrizzleDB) {}

  // CSVエクスポート（日本郵便フォーマット）/ CSV导出（日本邮便格式）
  async exportCsv(tenantId: string, dto: Record<string, any>) {
    ...
    // CSVヘッダー行 / CSV头行
    const headerRow = JAPAN_POST_CSV_HEADERS.join(',');
    ...
    const dataRows = orders.map((order: any) => {
      const fields = [
        ...
        this.escapeCsvField(order.productName ?? ''),
