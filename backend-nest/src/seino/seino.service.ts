...
// 西濃運輸の送り状種類（静的参照データ） / 西浓运输送状类型（静态参考数据）
const INVOICE_TYPES = [
  { code: '1', name: 'カンガルー特急便 / 袋鼠特急便', description: '通常便 / 普通便' },
  { code: '2', name: 'カンガルーミニ便 / 袋鼠迷你便', description: '小型便 / 小型便' },
  { code: '3', name: 'カンガルー航空便 / 袋鼠航空便', description: '航空便 / 航空便' },
];

// 西濃CSVヘッダー定義 / 西浓CSV头定义
const SEINO_CSV_HEADERS = [
  'お客様管理番号',      // 客户管理编号
  'お届け先郵便番号',    // 收件人邮编
  'お届け先住所1',       // 收件人地址1
  'お届け先住所2',       // 收件人地址2
...
@Injectable()
export class SeinoService {
  constructor(@Inject(DRIZZLE) private readonly db: DrizzleDB) {}

  // CSVエクスポート（西濃フォーマット）/ CSV导出（西浓格式）
  async exportCsv(tenantId: string, dto: Record<string, any>) {
    ...
    // CSVヘッダー行 / CSV头行
    const headerRow = SEINO_CSV_HEADERS.join(',');
    ...
    const dataRows = orders.map((order: any) => {
      const fields = [
        ...
        this.escapeCsvField(`${order.recipientCity ?? ''}${order.recipientStreet ?? ''}${order.recipientBuilding ?? ''}`),
        ...
        this.escapeCsvField(`${order.senderCity ?? ''}${order.senderStreet ?? ''}${order.senderBuilding ?? ''}`),
        this.escapeCsvField(order.senderName ?? ''),
        this.escapeCsvField(order.senderPhone ?? ''),
        '0',  // 荷姿デフォルト / 包装形态默认值
        this.escapeCsvField(order.packageCount ?? '1'),
        this.escapeCsvField(order.totalWeight ?? ''),
