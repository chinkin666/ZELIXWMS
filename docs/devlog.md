...
**影響範囲 / 影响范围**: frontend 40+ files, backend-nest 30+ files
**関連ドキュメント / 关联文档**: TODOS.md, docs/migration/

### 内容 / 内容

#### 新機能 / 新功能
- 入庫ワークステーション + 管理者ダッシュボード / 入库工作台 + 管理者仪表盘
- 出荷停止（画面+CSV）、送り状再発行/追加発行 / 出货停止（画面+CSV）、运单重新发行/追加发行
- 通過型→自動出荷指示生成、7日保管超過チェック / 通过型→自动出货指示生成、7日保管超期检查
- 作業グループ5分組条件（県内外/顧客/SKU数/業務/SLA）/ 作业分组5种条件（县内外/客户/SKU数/业务/SLA）
- 保留注文7日自動削除 + 6日目アラート / 保留订单7日自动删除 + 第6天告警
- PDF帳票4種追加（入庫予定リスト/一覧表/ロケーション看板/在庫証明書）/ PDF帐票4种追加
- 拠点間在庫移動 / 跨仓库库存转移
- 日本郵便 + 西濃運輸 CSV連携モジュール / 日本邮便 + 西浓运输 CSV对接模块
- 送り状プリセットテンプレート4社 / 运单预设模板4家

#### 品質改善 / 品质改善
- 全量QA 137ページ: CRITICAL 5件 + HIGH 33件 + deferred 12件 修正
- MongoDB→NestJS扁平フィールド適配 37箇所 / MongoDB→NestJS扁平字段适配37处
- 缺失API端点14個補完 / 缺失API端点14个补完
- yamatoB2 NestJS直接統合（Express依存排除）/ yamatoB2 NestJS直接集成（排除Express依赖）
- セキュリティ: SQL injection防止 + 原子ガード追加 / 安全: SQL注入防止 + 原子守卫追加

#### インフラ / 基础设施
- Docker PostgreSQL + seed data（商品24/在庫29/出荷10/入庫5）
- .env.production.example テンプレート作成 / .env.production.example 模板创建

---

## [2026-03-22] 入庫ワークステーション + 管理者ダッシュボード実装

**変更種別 / 变更类型**: feat
**影響範囲 / 影响范围**: frontend/src/views/workstation/, frontend/src/router/, frontend/src/components/layout/menuData.ts, backend-nest/src/kpi/
**関連ドキュメント / 关联文档**: TODOS.md

### 内容 / 内容

#### 入庫ワークステーション / 入库工作台 (`/inbound/workstation`)
- 出荷ワークステーションと同じデザインパターン / 与出货工作台相同的设计模式
- アラート：期限超過・検品待ち・棚入れ待ち / 告警：逾期·待检品·待上架
- メインアクション：入庫検品開始・棚入れ・指示作成・CSV取込 / 主操作：开始入库检品·上架·创建指示·CSV导入
- ステータスサマリー 6列：総数・下書き・検品待ち・入庫中・完了・期限超過 / 状态概览6列
- 最近入庫指示テーブル（進捗バー付き）/ 最近入库指示表（带进度条）
- クイックリンク：履歴・サイズ登録・請求・写真・通過型 / 快速链接

#### 管理者ダッシュボード / 管理者仪表盘 (`/manager/dashboard`)
- 概要4カード：出荷注文数・入庫注文数・商品数・総在庫数量 / 概要4卡片
- パフォーマンスKPI 4指標：フルフィルメント率・日次スループット・平均処理時間・返品数 / 性能KPI 4指标
- 出荷ステータス詳細（プログレスバー付き）/ 出货状态详情（带进度条）
- 在庫状況詳細（利用可能率プログレス付き）/ 库存状况详情（带可用率进度条）
- 低在庫商品テーブル / 低库存商品表
- クイックナビゲーション 8リンク / 快速导航8链接

#### バックエンド / 后端
- KpiController に 5 端点追加：overview-metrics, shipment-metrics, inventory-metrics, performance, alerts
- KpiService の既存メソッドを公開（新コード不要）/ 公开KpiService已有方法（无需新代码）

#### ルーター・メニュー / 路由·菜单
- `/inbound` リダイレクト先を `/inbound/workstation` に変更
- `/manager/dashboard` ルート追加
- メインメニューに「管理者」追加、サブメニュー3項目
- 入庫サブメニューに「ワークステーション」追加
