...
export function useV2HoldDelete({ state, draftStore, heldRowIds, allRows, loadBackendOrders }: HoldDeleteDeps) {
  ...
  const handleHold = () => {
    ...
    toast.success(`${ids.length}件を保留に設定しました`)
  ...
  const handleHoldBackend = async () => {
    ...
    try {
      ...
      toast.error(err?.message || '保留設定に失敗しました')
    }
    state.selectedRows.value = []
  }
  ...
  const handleReleaseHold = async () => {
    ...
    state.selectedRows.value = []
    toast.success(`${count}件の保留を解除しました`)
  }
  ...
  const handleUnconfirm = async () => {
    ...
    try {
      ...
      await loadBackendOrders()
      toast.success(`${ids.length}件の確認を取り消しました`)
    } catch (err: any) {
      toast.error(err?.message || '確認取消に失敗しました')
    }
    state.selectedRows.value = []
  }
  ...
  const handleDeleteBackend = async () => {
    ...
    try {
      ...
      toast.success(`${result.deletedCount}件の出荷指示を削除しました`)
      state.selectedRows.value = []
      await loadBackendOrders()
    } catch (e: any) {
      toast.error(e?.message || '削除に失敗しました')
