...
export async function fetchScript(id: string): Promise<AutomationScript> {
  const response = await apiFetch(`${API_BASE_URL}/extensions/scripts/${id}`)
  if (!response.ok) throw new Error(`スクリプトの取得に失敗しました: ${response.statusText}`)
  return response.json()
}

export async function createScript(data: Partial<AutomationScript>): Promise<AutomationScript> {
  const response = await apiFetch(`${API_BASE_URL}/extensions/scripts`, {
    method: 'POST',
...
export async function deleteScript(id: string): Promise<void> {
  const response = await apiFetch(`${API_BASE_URL}/extensions/scripts/${id}`, { method: 'DELETE' })
  if (!response.ok) {
    const body = await response.json().catch(() => ({}))
...
export async function toggleScript(id: string): Promise<{ _id: string; enabled: boolean; message: string }> {
  const response = await apiFetch(`${API_BASE_URL}/extensions/scripts/${id}/toggle`, { method: 'POST' })
  if (!response.ok) {
    const body = await response.json().catch(() => ({}))
...
export async function validateScriptCode(code: string): Promise<{ valid: boolean; error?: string }> {
  ...
  const response = await apiFetch(`${API_BASE_URL}/extensions/scripts/validate/validate`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
export async function testScript(id: string, payload?: Record<string, unknown>): Promise<{
  ...
  const response = await apiFetch(`${API_BASE_URL}/extensions/scripts/${id}/test`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
