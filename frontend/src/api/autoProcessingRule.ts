...
export async function fetchAutoProcessingRules(): Promise<AutoProcessingRule[]> {
  const response = await apiFetch(`${API_BASE_URL}/extensions/auto-processing-rules`)
  if (!response.ok) {
...
export async function fetchAutoProcessingRule(id: string): Promise<AutoProcessingRule> {
  const response = await apiFetch(`${API_BASE_URL}/extensions/auto-processing-rules/${id}`)
  if (!response.ok) {
...
export async function reorderAutoProcessingRules(orderedIds: string[]): Promise<void> {
  const response = await apiFetch(`${API_BASE_URL}/extensions/auto-processing-rules/reorder`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
export async function runAutoProcessingRule(id: string): Promise<{ message: string; data: ManualRunResult }> {
  const response = await apiFetch(`${API_BASE_URL}/extensions/auto-processing-rules/${id}/run`, {
    method: 'POST',
  })
  if (!response.ok) {
    const json = await response.json().catch(() => ({}))
