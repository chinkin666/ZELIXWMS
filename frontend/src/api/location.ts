...
export async function fetchLocation(id: string): Promise<Location> {
  const res = await apiFetch(`${API_BASE_URL}/locations/${id}`)
...
export async function deleteLocation(id: string): Promise<void> {
  const res = await apiFetch(`${API_BASE_URL}/locations/${id}`, { method: 'DELETE' })
...
export async function seedLocations(): Promise<{ message: string; created: string[] }> {
  const res = await apiFetch(`${API_BASE_URL}/locations/seed`, { method: 'POST' })
