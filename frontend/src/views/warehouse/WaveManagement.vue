<template>
  <div class="wave-management">
    <ControlPanel title="ウェーブ管理" :show-search="false">
      <template #actions>
        <OButton variant="primary" @click="openCreate">新規ウェーブ</OButton>
      </template>
    </ControlPanel>
    ...
    <div class="search-section">
      ...
      <select v-model="filterStatus" class="o-input" style="width: 160px;">
        ...
        <option value="sorting">仕分け中</option>
    ...
    <div class="o-table-wrapper">
      <table class="o-table">
        <thead>
          <tr>
            <th class="o-table-th" style="width: 160px">ウェーブ番号</th>
            <th class="o-table-th" style="width: 120px">ステータス</th>
            <th class="o-table-th" style="width: 90px">優先度</th>
            <th class="o-table-th" style="width: 90px">出荷件数</th>
            <th class="o-table-th" style="width: 120px">担当者</th>
            <th class="o-table-th" style="width: 160px">開始日時</th>
            <th class="o-table-th" style="width: 160px">完了日時</th>
          ...
          <tr v-for="w in waves" :key="w._id" class="o-table-row">
            ...
            <td class="o-table-td o-table-td--actions">
              <template v-if="w.status === 'draft'">
                <OButton variant="success" size="sm" @click="handleStart(w)">開始</OButton>
              ...
              <template v-else-if="w.status === 'picking' || w.status === 'sorting' || w.status === 'packing'">
                <OButton variant="success" size="sm" @click="handleComplete(w)">完了</OButton>
              </template>
            </td>
          </tr>
        </tbody>
    ...
    <!-- Create/Edit Dialog -->
    <ODialog v-model="dialogOpen" :title="isEditing ? 'ウェーブを編集' : 'ウェーブを追加'" size="lg" @confirm="handleSave">
      <div class="form-grid">
        <div class="form-field">
          <label class="form-label">ウェーブ番号 <span class="required">*</span></label>
...
// Labels & tag classes
const statusLabel = (status: Wave['status']): string => {
...
const priorityLabel = (priority: Wave['priority']): string => {
...
const confirmDelete = (w: Wave) => {
  if (w.status !== 'draft') {
    showToast('下書きステータスのウェーブのみ削除できます', 'danger')
    return
  }
  if (!confirm(`「${w.waveNumber}」を削除しますか？`)) return
  deleteWave(w._id)
    .then(async () => {
...
const handleStart = async (w: Wave) => {
  try {
    ...
    showToast('ウェーブを開始しました', 'success')
    await loadList()
  } catch (error: any) {
    showToast(error?.message || '開始に失敗しました', 'danger')
  }
}
...
const handleComplete = async (w: Wave) => {
  try {
    ...
    showToast('ウェーブを完了しました', 'success')
    await loadList()
  } catch (error: any) {
    showToast(error?.message || '完了に失敗しました', 'danger')
  }
}
