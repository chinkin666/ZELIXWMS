<template>
  ...
  >
    ...
    <div class="print-preview-section">
      <div v-if="printRendering" class="rendering">{{ t('wms.inspection.rendering', 'レンダリング中...') }}</div>
      <div v-else-if="printError" class="error">{{ printError }}</div>
      <div v-else-if="!printImageUrl" class="placeholder">{{ t('wms.inspection.generatingPreview', '印刷プレビューを生成中...') }}</div>
