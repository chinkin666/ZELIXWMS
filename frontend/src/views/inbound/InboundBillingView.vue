...
const summaryAmount = computed(() => billingRows.value.reduce((sum, r) => sum + (r.workCharge || 0), 0))
...
const tableColumns = computed<TableColumn[]>(() => [
  { key: 'date', dataKey: 'date', title: t('wms.inbound.date', '入庫日'), width: 120, fieldType: 'date',
    cellRenderer: ({ rowData }: { rowData: BillingRow }) => formatDate(rowData.date) },
  { key: 'inboundNumber', dataKey: 'inboundNumber', title: t('wms.inbound.number', '入庫番号'), width: 160, fieldType: 'string' },
  { key: 'customerName', dataKey: 'customerName', title: t('wms.common.customerName', '顧客名'), width: 160, fieldType: 'string' },
  { key: 'productCount', dataKey: 'productCount', title: t('wms.inbound.productCount', '商品数'), width: 100, fieldType: 'number' },
  { key: 'caseCount', dataKey: 'caseCount', title: t('wms.inbound.caseCount', 'ケース数'), width: 100, fieldType: 'number' },
  { key: 'workCharge', dataKey: 'workCharge', title: t('wms.billing.workCharge', '作業料金'), width: 120, fieldType: 'number',
    cellRenderer: ({ rowData }: { rowData: BillingRow }) => `¥${(rowData.workCharge || 0).toLocaleString()}` },
])
...
const loadBilling = async () => {
  try {
    ...
    const res = await apiFetch(`${API_BASE_URL}/billing/work-charges?category=inbound&year=${year}&month=${month}`)
...
<style scoped>
.inbound-billing { display: flex; flex-direction: column; padding: 0 20px 20px; gap: 16px; }
