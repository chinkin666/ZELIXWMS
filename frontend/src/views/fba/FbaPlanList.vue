<template>
  <div class="fba-plan-list">
    <ControlPanel title="FBAプラン一覧" :show-search="false">
      <template #actions>
        <OButton variant="primary" size="sm" @click="$router.push('/fba/plans/create')">新規作成</OButton>
      </template>
    </ControlPanel>
...
import type { FbaShipmentPlan, FbaPlanStatus } from '@/api/fba'

const router = useRouter()
const toast = useToast()

const rows = ref<FbaShipmentPlan[]>([])
const total = ref(0)
const currentPage = ref(1)
const pageSize = ref(25)
const currentStatus = ref<FbaPlanStatus | ''>('')

// ステータスタブ定義 / 状态标签定义
const statusTabs: ReadonlyArray<{ readonly label: string; readonly value: FbaPlanStatus | '' }> = [
  { label: '全て', value: '' },
...
// ステータスラベル / 状态标签
const statusLabel = (s: FbaPlanStatus): string => ({
  draft: '下書き',
  confirmed: '確定',
  shipped: '出荷済',
  received: '受領済',
...
const statusClass = (s: FbaPlanStatus): string => ({
...
const tableColumns: TableColumn[] = [
  {
    ...
    width: 160,
    fieldType: 'string',
    cellRenderer: ({ rowData }: { rowData: FbaShipmentPlan }) =>
      h('span', { class: 'plan-number clickable', onClick: () => router.push(`/fba/plans/${rowData._id}/edit`) }, rowData.planNumber),
  },
  {
    key: 'destinationFc',
    dataKey: 'destinationFc',
    title: 'FC',
    width: 100,
    fieldType: 'string',
  },
  ...
  },
  {
    ...
    width: 110,
    fieldType: 'date',
    cellRenderer: ({ rowData }: { rowData: FbaShipmentPlan }) => formatDate(rowData.createdAt),
  },
  {
    key: 'actions',
    title: '操作',
    ...
    cellRenderer: ({ rowData }: { rowData: FbaShipmentPlan }) => {
      ...
      if (rowData.status === 'draft') {
        buttons.push(
          h(OButton, { size: 'sm', variant: 'secondary', onClick: () => router.push(`/fba/plans/${rowData._id}/edit`) }, () => '編集'),
        ...
        buttons.push(
          h(OButton, { size: 'sm', variant: 'secondary', onClick: () => router.push(`/fba/plans/${rowData._id}/edit`) }, () => '詳細'),
        )
      }
...
function filterByStatus(status: FbaPlanStatus | '') {
